...
AuthenFriend::AuthenFriend(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::AuthenFriend), _label_point(5, 6)
{
    ui->setupUi(this);
...
void AuthenFriend::SlotApplySure()
{
    //发送请求逻辑
    QJsonObject jsonObj;
    auto uid = UserMgr::GetInstance()->GetUid(); //获取当前用户的uid, 作为发起好友申请的用户标识
    jsonObj["fromuid"] = uid;
    jsonObj["touid"] = _apply_info->_uid;
    QString back_name = "";
    //如果好友备注输入框的文本为空, 则使用占位文本作为好友备注
    if(ui->back_ed->text().isEmpty()){
        back_name = ui->back_ed->placeholderText();
    }else{
        back_name = ui->back_ed->text(); //获取好友备注输入框的文本作为好友备注
    }
    jsonObj["back"] = back_name;

    QJsonDocument doc(jsonObj); //将JSON对象转换为JSON文档, 以便序列化为JSON字符串
    QByteArray jsonData = doc.toJson(QJsonDocument::Compact); //将JSON文档序列化为紧凑格式的JSON字符串
